...
import addStore from "../../../redux/thunk/store/addStore";
...
const AddStore = () => {
  ...
  function handleAddStoreThumbnail(event) {
    const formData = new FormData();
    formData.append("thumbnail", event.target.files[0]);

    const uploadBannerThumbnail = async () => {
      setLoading(true);
      const request = await fetch(`https://canim.onrender.com/store/thumbnail`, {
        method: "POST",
        body: formData,
      });
      ...
      >
        ...
        <div class="w-full">
          ...
          <input
            ...
            placeholder="Write your store title"
            required
          />
        </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <input
                ...
                type="file"
                name="thumbnail"
                onChange={handleAddStoreThumbnail}
              />
            </>
          )}
        </div>
